...
export const ModalReservar = () => {
    ...
    useEffect(() => {
        JX.consultar("SELECT CODVEICULO, PLACA, MARCAMODELO, AD_MARCA FROM TGFVEI WHERE AD_CARROTRACT = 'S'")
            .then((data: any) => {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        if (hora_saida && hora_chegada) {
            ...
            if (horaChegadaInMinutes <= horaSaidaInMinutes) {
                form.setError("hora_chegada", {
                    message: "A Hora de Chegada não pode ser igual ou anterior à Hora de Saída",
                });
                return;
            }
        }
        ...
        if (dhCompletaSaida < dhCompletaAtual) {
            form.setError("hora_saida", {
                message: "O horário de saída não pode ser anterior ao horário atual de hoje.",
            });
            return;
        }
        ...
        const consulta = `
            ...
            AND (
                ('${String(dataCompletaSaida)}' > DH_SAIDA AND '${String(dataCompletaSaida)}' < DH_CHEGADA)
                OR
                ('${String(dataCompletaChegada)}' > DH_SAIDA AND '${String(dataCompletaChegada)}' < DH_CHEGADA)
                OR
                (DH_SAIDA >= '${String(dataCompletaSaida)}' AND DH_CHEGADA <= '${String(dataCompletaChegada)}')
                OR
                (DH_SAIDA < '${String(dataCompletaSaida)}' AND DH_CHEGADA > '${String(dataCompletaChegada)}')
            )    
        `
    ...
    const salvarReserva = async (values: z.infer<typeof formSchema>, dataCompletaSaida: string, dataCompletaChegada: string) => {
        try {
            ...
            if (response.status == "1") {
                ...
                toast({
                    ...
                    description: "Sua reserva foi enviada para aprovação. Aguarde enquanto processamos a solicitação.",
    ...
    return (
        <>
            <Dialog open={modalReservar} onOpenChange={(isOpen) => {
                if (!isOpen) {
                    closeModal();
                }
            }}>
                <DialogContent className="sm:max-w-[50%] ">
                    <DialogHeader>
                        <h1 className="font-semibold text-lg text-slate-700">REALIZAR PRÉ RESERVA</h1>
                    </DialogHeader>
                    <FormProvider {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)}>
                                            ...
                                            <FormControl>
                                                ...
                                                >
                                                    <SelectTrigger className="border-2 border-blue-950 rounded px-3 py-2 text-sm">
                                                        <SelectValue placeholder="Selecionar Veículo">{field.value ? `${listVeiculos.find((v) => v.CODVEICULO == String(field.value))?.PLACA}`
                                                            : <span className="text-gray-500">Selecione Veículo</span>}
                                                        </SelectValue>
                                                    </SelectTrigger>
                                                    <SelectContent className="bg-slate-300 uppercase">
                                                        {listVeiculos.map((veiculo: any) => (
                                                            ...
                                                            >
                                                                {veiculo.PLACA} {veiculo?.AD_MARCA ? `- ${veiculo.AD_MARCA}` : ''}
                                                            </SelectItem>
                                                        ))}
                                                    </SelectContent>
                                                </Select>
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 mt-3 ">
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem className="flex-1">
                                            ...
                                            <FormControl>
                                                <Textarea placeholder="" {...field} className="border-2 border-blue-950 border-dashed rounded-lg  h-[15vh]" />
                                            </FormControl>
                                            <FormMessage />
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem className="flex-1">
                                            <FormLabel>Inserir Anexo (NÃO INSERIR PDF APENAS ARQUIVO DE IMAGEM)</FormLabel>
                                            <FormControl>
